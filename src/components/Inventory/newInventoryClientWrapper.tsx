...
import { ColumnFilters, applyColumnFilters, applyVariationFilters } from "@/components/ListTable/ColumnFilters"
...
import NewInventoryHeader from "./HeaderSetion/NewInventoryHeader"
...
export default function NewInventoryClientWrapper({ initialProducts, categories, stores }: Props) {
    const { user } = useAuth()
    const {
        currentPage,
        editValue,
    ...
    // Apply column filters to products
    ...
    const flattenedProducts = useMemo<FlattenedItem[]>(() => {
        ...
        filteredProducts.forEach((product) => {
            const totalStockQuantity = product.ProductVariations.reduce((total: any, v: { stockQuantity: any }) => total + v.stockQuantity, 0)
