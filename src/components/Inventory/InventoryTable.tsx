        ...
        <div className="overflow-x-auto h-full">
          <Table>
            <TableHeader className="sticky top-0 bg-gray-50 dark:bg-slate-800">
              <TableRow>
                <TableHead className="whitespace-nowrap text-center font-semibold text-gray-700 dark:text-gray-200">PRODUCTO</TableHead>
                <TableHead className="whitespace-nowrap text-center font-semibold text-gray-700 dark:text-gray-200">CÓDIGO SKU</TableHead>
                <TableHead className="whitespace-nowrap text-center font-semibold text-gray-700 dark:text-gray-200">TALLA</TableHead>
                <TableHead className="whitespace-nowrap text-center font-semibold text-gray-700 dark:text-gray-200">PRECIO COSTO</TableHead>
                <TableHead className="whitespace-nowrap text-center font-semibold text-gray-700 dark:text-gray-200">PRECIO PLAZA</TableHead>
                <TableHead className="whitespace-nowrap text-center font-semibold text-gray-700 dark:text-gray-200">STOCK CENTRAL</TableHead>
                <TableHead className="whitespace-nowrap text-center font-semibold text-gray-700 dark:text-gray-200">STOCK AGREGADO</TableHead>
              </TableRow>
            </TableHeader>
            ...
            <TableBody>
                ...
                const stockAgregado = variation.StoreProducts?.filter((sp: any) => !adminStoreIDs.includes(sp.storeID)).reduce((sum: number, sp: any) => sum + sp.quantity,0) ?? 0
                const profitMargin =variation.priceList > 0 ? ((variation.priceList - variation.priceCost) / variation.priceList) * 100 : 0
                return (
                    <TableRow
                    ...
                    >
                        {isFirst && (
                            <MotionItem key={`${product.productID}-${variation.variationID}`} delay={index + 3}>
                        ...
                        >
                        {editingField?.sku === variation.sku && editingField?.field === "priceCost" ? (
                            <div className="flex justify-center">
                            <Input
                                value={editValue}
                                onChange={(e) => setEditValue(e.target.value)}
                        ...
                        <TableCell
                            ...
                            >
                            {editingField?.sku === variation.sku && editingField?.field === "priceList" ? (
                                <div className="flex justify-center">
                                <Input
                                    value={editValue}
                                    onChange={(e) => setEditValue(e.target.value)}
                                ...
                                <span
                                    className={`text-xs ${profitMargin > 30 ? "text-green-600" : profitMargin > 15 ? "text-yellow-600" : "text-red-600"}`}
                                >
                        ...
                        <TableCell
                            ...
                            >
                            {editingField?.sku === variation.sku && editingField?.field === "stockQuantity" ? (
                                <div className="flex justify-center">
                                <Input
                                    value={editValue}
                                    onChange={(e) => setEditValue(e.target.value)}
